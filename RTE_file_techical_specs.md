...
### 7.2 Handling Empty IDs (Origin)
The First Record (Sequence 0) represents the Origin. Since no leg leads *to* the origin, its **ID Field (Offset 08)** is typically `0x00` (Empty/Null).

* **Importer Behavior:** If an ID field is found to be null (common for the first record), the parsing software must handle it gracefully (e.g., by displaying a placeholder or leaving it blank). The string "WP-1" is **not** present in the binary data.
